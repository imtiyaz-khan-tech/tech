...
let doc_title = 'FLS - PermissionSets';
let conn;
...
async function getAllPermissionSets(){
    ...
    let q = `SELECT ${fields_array.join(',')} FROM PermissionSet Where ProfileId = Null AND PermissionSetGroupId = Null Order By Name`;
...
function getFieldPermissions(){
    ...
    let q = `SELECT ${fields_array.join(',')} FROM FieldPermissions Where SobjectType = '${selectedObject}' AND Parent.Name = '${selectedPermissionset}'`;
